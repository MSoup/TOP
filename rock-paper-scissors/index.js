...
const subHeading = "CHOOSE YOUR WEAPON";
...
const playRound = function (playerSelection) {
  ...
  if (WIN_PAIRINGS[choice] == computerSelection) {
    playerScore += 1;
    displayResult.textContent = `You Win! :) ${capitalize(
      choice
  ...
  // Player loses
    ...
    computerScore += 1;
    displayResult.textContent = `You Lose! AI chose ${capitalize(
      computerSelection
    )}. ${capitalize(computerSelection)} beats ${capitalize(choice)}`;
  }
  // Tie
  else {
...
buttons.forEach((button) => {
  button.addEventListener("click", () => buttonFunction(button.value));
});
